...
def compute_paycheck_schedule(ret):
    """Compute (net_per_check, paydays_per_month, monthly_totals) from a settings dict."""
    if not ret or not ret.get("hourly_rate") or not ret.get("next_payday"):
        return 0.0, {}, {}
    gross_annual = float(ret["hourly_rate"]) * float(ret.get("hours_per_year", 2080))
    freq = ret.get("pay_frequency", "Biweekly")
    checks = {"Biweekly": 26, "Semi-Monthly": 24, "Monthly": 12}.get(freq, 26)
    gross_pc = gross_annual / checks
    k401_pct = float(ret.get("k401_contribution_pct", 5.0)) / 100.0
    k401_pc = gross_pc * k401_pct
    k401_annual = gross_annual * k401_pct
    benefits = float(ret.get("benefits_per_check", 0))
    ...
    try:
        start = datetime.strptime(ret["next_payday"], "%Y-%m-%d").date()
    except Exception:
        return 0.0, {}, {}
    if freq == "Biweekly":
...
def build_projection_frames(holdings_by_account, live_prices, cash_balance, growth_rate,
                            savings_yield, include_shared, budget_months,
                            share_alloc_pcts, total_months=NUM_MONTHS):
    """Return (balances_df, shares_df) for the 12-month portfolio simulation."""
    ...
    for acct_name in sorted(holdings_by_account.keys()):
        ...
        for h in holdings_by_account[acct_name]:
            ticker = h["ticker"]
            price = float(live_prices.get(ticker, h["price"]))
            is_mm = ticker in MONEY_MARKET_TICKERS
            sim_state[acct_name].append({
                "ticker": ticker,
                "shares": float(h["shares"]),
                "price": price,
                "value": float(h["value"]) if is_mm else float(h["shares"]) * price,
                "yield_pct": float(h.get("yield_pct", 0.0)),
                "is_mm": is_mm,
            })
    balance_rows = []
    share_rows = []
    for month_idx in range(total_months + 1):
        row = {"MonthIndex": month_idx}
        share_row = {"MonthIndex": month_idx}
        grand_total = 0.0
        for acct_name, holdings in sim_state.items():
            if month_idx > 0:
                for holding in holdings:
                    if holding["is_mm"]:
                        holding["value"] *= (1 + savings_yield / 12)
                    else:
                        if holding["yield_pct"] > 0.02 and holding["price"] > 0:
                            dividend = holding["shares"] * holding["price"] * holding["yield_pct"] / 12
                            holding["shares"] += dividend / holding["price"]
                        holding["price"] *= monthly_growth
            if month_idx > 0 and month_idx - 1 < len(budget_months):
                src_month = budget_months[month_idx - 1]
                contribution = 0.0
                for inv_field, portfolio in INVEST_TO_PORTFOLIO.items():
                    if portfolio == acct_name and acct_name != "Shared":
                        contribution += float(src_month.get(inv_field, 0.0))
                if contribution > 0:
                    allocations = share_alloc_pcts.get(acct_name, {})
                    allocated = False
                    for holding in holdings:
                        pct = float(allocations.get(holding["ticker"], 0.0))
                        if pct <= 0:
                            continue
                        dollars = contribution * pct / 100.0
                        if holding["is_mm"]:
                            holding["value"] += dollars
                            allocated = True
                        elif holding["price"] > 0:
                            holding["shares"] += dollars / holding["price"]
                            allocated = True
                    if not allocated:
                        for holding in holdings:
                            if holding["is_mm"]:
                        ...
                        if not allocated:
                            for holding in holdings:
                                if not holding["is_mm"] and holding["price"] > 0:
                                    holding["shares"] += contribution / holding["price"]
                                    break
            acct_total = sum(
                holding["value"] if holding["is_mm"] else holding["shares"] * holding["price"]
                for holding in holdings
            )
            row[acct_name] = acct_total
            grand_total += acct_total
            for holding in holdings:
                if not holding["is_mm"]:
                    share_row[f"{holding['ticker']} ({acct_name})"] = holding["shares"]
        row["Cash"] = cash_balance
        row["Total"] = grand_total + cash_balance
        balance_rows.append(row)
        share_rows.append(share_row)
    balances_df = pd.DataFrame(balance_rows).fillna(0.0)
    shares_df = pd.DataFrame(share_rows).fillna(0.0)
    return balances_df, shares_df


def compute_month_totals(month):
    income_total = sum(float(month.get(field, 0.0)) for field in INCOME_FIELDS)
    expense_total = sum(float(month.get(field, 0.0)) for field in EXPENSE_FIELDS) + float(month.get("one_time_expense", 0.0))
    invested_total = sum(float(month.get(field, 0.0)) for field in INVEST_FIELDS)
...
def save_projection():
    data = {
        ...
        "savings_yield": st.session_state.get("savings_yield"),
    }
...
def load_projection():
    if PROJECTION_FILE.exists():
        ...
        if "growth_rate" in data and data["growth_rate"] is not None:
            st.session_state.growth_rate = float(data["growth_rate"])
        if "savings_yield" in data and data["savings_yield"] is not None:
            st.session_state.savings_yield = float(data["savings_yield"])
def save_dividends():
